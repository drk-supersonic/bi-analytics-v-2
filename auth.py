...
def create_user(username: str, password: str, role: str, email: Optional[str] = None, created_by: Optional[str] = None) -> bool:
    ...
    with st.sidebar:
        ...
        if has_report_access(user['role']):
            if current_page == "reports":
                st.button("📊 Отчеты", use_container_width=True, type="primary", disabled=True, help="Текущая страница")
            else:
            ...
            if current_page == "reports":
                ...
                reason_reports = ["Динамика отклонений по месяцам", "Динамика отклонений", "Динамика причин отклонений"]
        ...
        if has_admin_access(user['role']):
            ...
            if current_page == "admin":
                st.button("⚙️ Общие настройки", use_container_width=True, type="primary", disabled=True, help="Текущая страница")
            else:
        ...
        if user['role'] in ['analyst', 'admin', 'superadmin']:
            if current_page == "analyst_params":
                st.button("📝 Параметры отчетов", use_container_width=True, type="primary", disabled=True, help="Текущая страница")
            else:
